...
A BOSS backend owner must review and install `backend/001_rooms.sql` through the normal migration process. **This repository does not deploy it automatically.** The migration expects BOSS `organisations`, `organisation_members`, `auth.users` and `auth.uid()`. It adds RPC-only room/message/memory tables and `boss_rooms_v1(operation, payload)`; clients use the existing authenticated `SupabaseDataProvider`, never a service-role key.
...
Current limitations: polling rather than realtime notifications; newest 100-message pages with explicit earlier-page loading; drafts and unconfirmed send IDs live only for the open tab; no durable offline send queue, attachments, calls or background scheduling. A single tab runs one assistant at a time. Lost send acknowledgements can be retried with the same request ID while the tab remains open. Do not claim guaranteed delivery across app restarts.
